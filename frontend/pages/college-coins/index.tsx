...
const CollegeCoinCard = ({
  ...
  useEffect(() => {
    if (!isMining || !sessionStart || !sessionRate) {
      ...
      if (totalRef.current) totalRef.current.textContent = balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 });
    ...
    const tick = () => {
      ...
      if (totalRef.current) totalRef.current.textContent = (balance + earned).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 });
